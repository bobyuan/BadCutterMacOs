...
| 2026-07-21 | **CRITICAL: relaunch showed pin-less scores (load-ordering bug)** — diagnosed from the user's diagnostic logs: at 13:38 UTC (fresh launch) every pin vanished from the displayed chain while ledger.jsonl held all 41 pin events intact (run 1, gate seq 0). `applyLoadedSession` ran `computeAllScores()` BEFORE `refreshSessionDerivedState()` (which derives serveOverrides/winnerOverrides/adjustments from the ledger), so post-launch scores ignored all pins until the next recompute — and corrections made against that stale display re-pinned stale values over the user's real ones (why corrections "didn't stick" and had to be redone). Fixed: refresh before compute. Also: **"Set score before this play…"** (user ask) — `scoreAdjustedBefore` event rebases the chain AT a play's serve (works for a game's first play too); menu + alert + History + corrections-log entries. |
| 2026-07-21 | **G2+G4+G5+G6+G7 implemented** ("go ahead on all"). G5: `shuttleCentroid` — first ≤5 cached TrackNet positions near the serve moment are primary serve-side evidence (no video decode); motion centroid fallback only. G4: `serveAnchorTime` — sampling window centered on the first audio onset after play start. Per-evidence-source largest-gap splits (shuttle/motion biases not mixed). G6: incremental detections reuse the persisted axis. G2: `inferSides` — DP over (play, side, A-wins) maximizing margin-weighted evidence over LEGAL chains only (no play after terminal); pins = hard constraints; unknowns filled from context (+small switch bonus, from 9/9-wrong-guess evidence); algorithm validated against a Python reference on 4 scenarios incl. the 23:9 repair. G7: winner provenance badges per row (📌 pinned / ✨ inferred / ❓ guessed; detected clean) + `[inferred]` tier in provenance/logs; `serveInferredIDs` persisted in baseline. Also verified (user ask): scores are fully run-scoped — re-analysis → fresh score set in the new run; switching runs restores that run's scores/pins (SCORING.md §12). Tests written (inference repair/fill/pins, shuttle centroid, onset anchor); suite deferred (app open); DP logic Python-validated. |
| 2026-07-21 | **G8 + G9 done — SCORING.md gap list closed** (G10 residual-low). G8: margins = distance to split boundary normalized by mean intra-cluster spread. G9: "Release manual score edits" context item — release sentinels (side "unknown" / negative scores) remove the winner pin, final-play winner, and manual score sets via last-wins ledger events; play returns to detection+inference; History describes releases. Also fixed shuttle-serve evidence: only the FIRST CONTINUOUS tracking stretch counts (a gap = later rally shot; test caught the dilution). **Full suite sweep green: ShadowEval 30/30, SessionStore, HighlightScorer, SegmentUtils.** |
